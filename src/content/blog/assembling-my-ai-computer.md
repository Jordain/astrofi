...
<!-- [list of the parts](https://ca.pcpartpicker.com/list/vhLwTn#compatibility_notes/) -->
...
From AI dreams to owning my own machine, <br> I've upgraded from 
<span class="relative group" style="display: inline-block;">
  <span class="text-blue-500 relative z-10" id="hover-text" >daydreaming ( •_•)>⌐■-■</span>
</span>

<script>
  const hoverText = document.getElementById('hover-text');
  const face = "daydreaming ( •_•)>⌐■-■";
  const sunglasses = "(⌐■_■) to ‘data’-scheming!";

  hoverText.addEventListener('mouseenter', () => {
    hoverText.textContent = face.replace(/\( •_•\)>⌐■-■/, sunglasses);
  });

  hoverText.addEventListener('mouseleave', () => {
    hoverText.textContent = face;
  });
</script>
